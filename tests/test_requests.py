...
import hackurllib
import pytest
from src.hackurllib.adapters import HTTPAdapter
from hackurllib.auth import HTTPDigestAuth, _basic_auth_str
from hackurllib.compat import (
    Morsel, cookielib, getproxies, str, urlparse,
    builtin_str)
from src.hackurllib.cookies import (
    cookiejar_from_dict, morsel_to_cookie)
from src.hackurllib.exceptions import (
    ConnectionError, ConnectTimeout, InvalidSchema, InvalidURL,
    MissingSchema, ReadTimeout, Timeout, RetryError, TooManyRedirects,
    ProxyError, InvalidHeader, UnrewindableBodyError, SSLError, InvalidProxyURL, InvalidJSONError)
from hackurllib.models import PreparedRequest
from hackurllib.structures import CaseInsensitiveDict
from src.hackurllib.sessions import SessionRedirectMixin
from hackurllib.models import urlencode
from src.hackurllib.hooks import default_hooks
from hackurllib.compat import MutableMapping
...
class TestRequests:
    ...
    def test_entry_points(self):
        ...
        hackurllib.session
        hackurllib.session().get
        hackurllib.session().head
        hackurllib.get
        hackurllib.head
        hackurllib.put
        hackurllib.patch
        ...
        from hackurllib.packages.urllib3.poolmanager import PoolManager
    ...
    def test_no_content_length(self, httpbin, method):
        req = hackurllib.Request(method, httpbin(method.lower())).prepare()
    ...
    def test_empty_content_length(self, httpbin, method):
        req = hackurllib.Request(method, httpbin(method.lower()), data='').prepare()
        assert req.headers['Content-Length'] == '0'
    ...
    def test_override_content_length(self, httpbin):
        ...
        r = hackurllib.Request('POST', httpbin('post'), headers=headers).prepare()
        assert 'Content-Length' in r.headers
        assert r.headers['Content-Length'] == 'not zero'

    def test_path_is_not_double_encoded(self):
        request = hackurllib.Request('GET', "http://0.0.0.0/get/test case").prepare()
    ...
    def test_params_are_added_before_fragment(self, url, expected):
        request = hackurllib.Request('GET', url, params={"a": "b"}).prepare()
        assert request.url == expected

    def test_params_original_order_is_preserved_by_default(self):
        param_ordered_dict = collections.OrderedDict((('z', 1), ('a', 1), ('k', 1), ('d', 1)))
        session = hackurllib.Session()
        request = hackurllib.Request('GET', 'http://example.com/', params=param_ordered_dict)
        prep = session.prepare_request(request)
        assert prep.url == 'http://example.com/?z=1&a=1&k=1&d=1'

    def test_params_bytes_are_encoded(self):
        request = hackurllib.Request('GET', 'http://example.com',
                                     params=b'test=foo').prepare()
        assert request.url == 'http://example.com/?test=foo'

    def test_binary_put(self):
        request = hackurllib.Request('PUT', 'http://example.com',
                                     data=u"ööö".encode("utf-8")).prepare()
        assert isinstance(request.body, bytes)

    def test_whitespaces_are_removed_from_url(self):
        # Test for issue #3696
        request = hackurllib.Request('GET', ' http://example.com').prepare()
        assert request.url == 'http://example.com/'

    @pytest.mark.parametrize('scheme', ('http://', 'HTTP://', 'hTTp://', 'HttP://'))
    def test_mixed_case_scheme_acceptable(self, httpbin, scheme):
        s = hackurllib.Session()
        s.proxies = getproxies()
        parts = urlparse(httpbin('get'))
        url = scheme + parts.netloc + parts.path
        r = hackurllib.Request('GET', url)
        r = s.send(r.prepare())
        assert r.status_code == 200, 'failed for scheme {}'.format(scheme)

    def test_HTTP_200_OK_GET_ALTERNATIVE(self, httpbin):
        r = hackurllib.Request('GET', httpbin('get'))
        s = hackurllib.Session()
        s.proxies = getproxies()
    ...
    def test_HTTP_302_ALLOW_REDIRECT_GET(self, httpbin):
        r = hackurllib.get(httpbin('redirect', '1'))
        assert r.status_code == 200
        assert r.history[0].status_code == 302
        assert r.history[0].is_redirect

    def test_HTTP_307_ALLOW_REDIRECT_POST(self, httpbin):
        r = hackurllib.post(httpbin('redirect-to'), data='test', params={'url': 'post', 'status_code': 307})
        assert r.status_code == 200
    ...
    def test_HTTP_307_ALLOW_REDIRECT_POST_WITH_SEEKABLE(self, httpbin):
        byte_str = b'test'
        r = hackurllib.post(httpbin('redirect-to'), data=io.BytesIO(byte_str), params={'url': 'post', 'status_code': 307})
        assert r.status_code == 200
        assert r.history[0].status_code == 307
        assert r.history[0].is_redirect
    ...
    def test_HTTP_302_TOO_MANY_REDIRECTS(self, httpbin):
        try:
            hackurllib.get(httpbin('relative-redirect', '50'))
    ...
    def test_http_301_changes_post_to_get(self, httpbin):
        r = hackurllib.post(httpbin('status', '301'))
        assert r.status_code == 200
    ...
    def test_http_301_doesnt_change_head_to_get(self, httpbin):
        r = hackurllib.head(httpbin('status', '301'), allow_redirects=True)
        print(r.content)
        assert r.status_code == 200
    ...
    def test_http_302_changes_post_to_get(self, httpbin):
        r = hackurllib.post(httpbin('status', '302'))
        assert r.status_code == 200
        assert r.request.method == 'GET'
        assert r.history[0].status_code == 302
        assert r.history[0].is_redirect
    ...
    def test_http_302_doesnt_change_head_to_get(self, httpbin):
        r = hackurllib.head(httpbin('status', '302'), allow_redirects=True)
        assert r.status_code == 200
        assert r.request.method == 'HEAD'
        assert r.history[0].status_code == 302
        assert r.history[0].is_redirect
    ...
    def test_http_303_changes_post_to_get(self, httpbin):
        r = hackurllib.post(httpbin('status', '303'))
        assert r.status_code == 200
        assert r.request.method == 'GET'
    ...
    def test_http_303_doesnt_change_head_to_get(self, httpbin):
        r = hackurllib.head(httpbin('status', '303'), allow_redirects=True)
        assert r.status_code == 200
        assert r.request.method == 'HEAD'
        assert r.history[0].status_code == 303
        assert r.history[0].is_redirect
    ...
    def test_header_and_body_removal_on_redirect(self, httpbin):
        ...
        ses = hackurllib.Session()
        req = hackurllib.Request('POST', httpbin('post'), data={'test': 'data'})
    ...
    def test_transfer_enc_removal_on_redirect(self, httpbin):
        ...
        ses = hackurllib.Session()
        req = hackurllib.Request('POST', httpbin('post'), data=(b'x' for x in range(1)))
        prep = ses.prepare_request(req)
        assert 'Transfer-Encoding' in prep.headers

        # Create Response to avoid https://github.com/kevin1024/pytest-httpbin/issues/33
        resp = hackurllib.Response()
    ...
    def test_fragment_maintained_on_redirect(self, httpbin):
        ...
        r = hackurllib.get(httpbin('redirect-to?url=get') + fragment)
    ...
    def test_HTTP_200_OK_GET_WITH_PARAMS(self, httpbin):
        ...
        r = hackurllib.get(httpbin('user-agent'), headers=heads)
    ...
    def test_HTTP_200_OK_GET_WITH_MIXED_PARAMS(self, httpbin):
        ...
        r = hackurllib.get(httpbin('get') + '?test=true', params={'q': 'test'}, headers=heads)
        assert r.status_code == 200
    ...
    def test_cookielib_cookiejar_on_redirect(self, httpbin):
        ...
        req = hackurllib.Request('GET', httpbin('headers'), cookies=cj)
        ...
        assert not isinstance(resp.request._cookies, src.hackurllib.cookies.RequestsCookieJar)
    ...
    def test_requests_in_history_are_not_overridden(self, httpbin):
        resp = hackurllib.get(httpbin('redirect/3'))
    ...
    def test_history_is_always_a_list(self, httpbin):
        ...
        resp = hackurllib.get(httpbin('get'))
        assert isinstance(resp.history, list)
        resp = hackurllib.get(httpbin('redirect/1'))
        assert isinstance(resp.history, list)
    ...
    def test_headers_on_session_with_None_are_not_sent(self, httpbin):
        ...
        req = hackurllib.Request('GET', httpbin('get'))
        prep = ses.prepare_request(req)
    ...
    def test_headers_preserve_order(self, httpbin):
        ...
        headers['Fifth'] = '5'
        headers['Second'] = '222'
        req = hackurllib.Request('GET', httpbin('get'), headers=headers)
        prep = ses.prepare_request(req)
    ...
    def test_user_agent_transfers(self, httpbin, key):
        ...
        heads = {key: 'Mozilla/5.0 (github.com/psf/hackurllib)'}

        r = hackurllib.get(httpbin('user-agent'), headers=heads)
        assert heads[key] in r.text

    def test_HTTP_200_OK_HEAD(self, httpbin):
        r = hackurllib.head(httpbin('get'))
        assert r.status_code == 200

    def test_HTTP_200_OK_PUT(self, httpbin):
        r = hackurllib.put(httpbin('put'))
        assert r.status_code == 200
    ...
    def test_BASICAUTH_TUPLE_HTTP_200_OK_GET(self, httpbin):
        ...
        r = hackurllib.get(url, auth=auth)
        assert r.status_code == 200

        r = hackurllib.get(url)
    ...
    def test_set_basicauth(self, httpbin, username, password):
        ...
        r = hackurllib.Request('GET', url, auth=auth)
        p = r.prepare()
    ...
    def test_basicauth_encodes_byte_strings(self):
        ...
        r = hackurllib.Request('GET', 'http://localhost', auth=auth)
        p = r.prepare()
    ...
    def test_errors(self, url, exception):
        with pytest.raises(exception):
            hackurllib.get(url, timeout=1)

    def test_proxy_error(self):
        # any proxy related error (address resolution, no route to host, etc) should result in a ProxyError
        with pytest.raises(ProxyError):
            hackurllib.get('http://localhost:1', proxies={'http': 'non-resolvable-address'})

    def test_proxy_error_on_bad_url(self, httpbin, httpbin_secure):
        with pytest.raises(InvalidProxyURL):
            hackurllib.get(httpbin_secure(), proxies={'https': 'http:/badproxyurl:3128'})

        with pytest.raises(InvalidProxyURL):
            hackurllib.get(httpbin(), proxies={'http': 'http://:8080'})

        with pytest.raises(InvalidProxyURL):
            hackurllib.get(httpbin_secure(), proxies={'https': 'https://'})

        with pytest.raises(InvalidProxyURL):
            hackurllib.get(httpbin(), proxies={'http': 'http:///example.com:8080'})

    def test_respect_proxy_env_on_send_self_prepared_request(self, httpbin):
        with override_environ(http_proxy=INVALID_PROXY):
            with pytest.raises(ProxyError):
                session = hackurllib.Session()
                request = hackurllib.Request('GET', httpbin())
    ...
    def test_respect_proxy_env_on_send_with_redirects(self, httpbin):
        with override_environ(http_proxy=INVALID_PROXY):
            with pytest.raises(ProxyError):
                ...
                print(url)
                request = hackurllib.Request('GET', url)
                session.send(request.prepare())
    ...
    def test_basicauth_with_netrc(self, httpbin):
        ...
        old_auth = src.hackurllib.sessions.get_netrc_auth

        try:
            def get_netrc_auth_mock(url):
                return auth
            src.hackurllib.sessions.get_netrc_auth = get_netrc_auth_mock
            ...
            r = hackurllib.get(url, auth=wrong_auth)
            assert r.status_code == 401
            ...
            src.hackurllib.sessions.get_netrc_auth = old_auth
    ...
    def test_DIGEST_STREAM(self, httpbin):
        ...
        for authtype in self.digest_auth_algo:
            ...
            r = hackurllib.get(url, auth=auth, stream=True)
            assert r.raw.read() != b''

            r = hackurllib.get(url, auth=auth, stream=False)
            assert r.raw.read() == b''
    ...
    def test_POSTBIN_GET_POST_FILES(self, httpbin):
        ...
        url = httpbin('post')
        hackurllib.post(url).raise_for_status()

        post1 = hackurllib.post(url, data={'some': 'data'})
        assert post1.status_code == 200

        with open('requirements-dev.txt') as f:
            post2 = hackurllib.post(url, files={'some': f})
        assert post2.status_code == 200

        post4 = hackurllib.post(url, data='[{"some": "json"}]')
        assert post4.status_code == 200

        with pytest.raises(ValueError):
            hackurllib.post(url, files=['bad file data'])

    def test_invalid_files_input(self, httpbin):

        url = httpbin('post')
        post = hackurllib.post(url,
    ...
    def test_POSTBIN_SEEKED_OBJECT_WITH_NO_ITER(self, httpbin):
        ...
        post1 = hackurllib.post(httpbin('post'), data=test)
        assert post1.status_code == 200
        assert post1.json()['data'] == 'test'

        test = TestStream('test')
        test.seek(2)
        post2 = hackurllib.post(httpbin('post'), data=test)
        assert post2.status_code == 200
    ...
    def test_POSTBIN_GET_POST_FILES_WITH_DATA(self, httpbin):
        ...
        with open('requirements-dev.txt') as f:
            post2 = hackurllib.post(url, data={'some': 'data'}, files={'some': f})
        assert post2.status_code == 200
    ...
    def test_post_with_custom_mapping(self, httpbin):
        ...
        url = httpbin('post')
        found_json = hackurllib.post(url, data=data).json().get('form')
    ...
    def test_conflicting_post_params(self, httpbin):
        ...
        with open('requirements-dev.txt') as f:
            with pytest.raises(ValueError):
                hackurllib.post(url, data='[{"some": "data"}]', files={'some': f})
            with pytest.raises(ValueError):
                hackurllib.post(url, data=u('[{"some": "data"}]'), files={'some': f})

    def test_request_ok_set(self, httpbin):
        r = hackurllib.get(httpbin('status', '404'))
        assert not r.ok

    def test_status_raising(self, httpbin):
        r = hackurllib.get(httpbin('status', '404'))
        with pytest.raises(src.hackurllib.exceptions.HTTPError):
            r.raise_for_status()

        r = hackurllib.get(httpbin('status', '500'))
        assert not r.ok

    def test_decompress_gzip(self, httpbin):
        r = hackurllib.get(httpbin('gzip'))
    ...
    def test_unicode_get(self, httpbin, url, params):
        hackurllib.get(httpbin(url), params=params)
    ...
    def test_pyopenssl_redirect(self, httpbin_secure, httpbin_ca_bundle):
        hackurllib.get(httpbin_secure('status', '301'), verify=httpbin_ca_bundle)

    def test_invalid_ca_certificate_path(self, httpbin_secure):
        INVALID_PATH = '/garbage'
        with pytest.raises(IOError) as e:
            hackurllib.get(httpbin_secure(), verify=INVALID_PATH)
        assert str(e.value) == 'Could not find a suitable TLS CA certificate bundle, invalid path: {}'.format(INVALID_PATH)

    def test_invalid_ssl_certificate_files(self, httpbin_secure):
        INVALID_PATH = '/garbage'
        with pytest.raises(IOError) as e:
            hackurllib.get(httpbin_secure(), cert=INVALID_PATH)
    ...
    def test_http_with_certificate(self, httpbin):
        r = hackurllib.get(httpbin(), cert='.')
        assert r.status_code == 200

    def test_https_warnings(self, nosan_server):
        """warnings are emitted with hackurllib.get"""
        ...
        with pytest.warns(None) as warning_records:
            ...
            hackurllib.get("https://localhost:{}/".format(port), verify=ca_bundle)
    ...
    def test_certificate_failure(self, httpbin_secure):
        ...
        with pytest.raises(SSLError):
            ...
            hackurllib.get(httpbin_secure('status', '200'))

    def test_urlencoded_get_query_multivalued_param(self, httpbin):

        r = hackurllib.get(httpbin('get'), params={'test': ['foo', 'baz']})
        assert r.status_code == 200
        assert r.url == httpbin('get?test=foo&test=baz')

    def test_form_encoded_post_query_multivalued_element(self, httpbin):
        r = hackurllib.Request(method='POST', url=httpbin('post'),
                               data=dict(test=['foo', 'baz']))
        prep = r.prepare()
        assert prep.body == 'test=foo&test=baz'

    def test_different_encodings_dont_break_post(self, httpbin):
        r = hackurllib.post(httpbin('post'),
    ...
    def test_unicode_method_name_with_request_object(self, httpbin):
        files = {'file': open(__file__, 'rb')}
        s = hackurllib.Session()
        req = hackurllib.Request(u('POST'), httpbin('post'), files=files)
    ...
    def test_hook_receives_request_arguments(self, httpbin):
        ...
        s = hackurllib.Session()
        r = hackurllib.Request('GET', httpbin(), hooks={'response': hook})
    ...
    def test_session_hooks_are_used_with_no_request_hooks(self, httpbin):
        ...
        r = hackurllib.Request('GET', httpbin())
        prep = s.prepare_request(r)
    ...
    def test_session_hooks_are_overridden_by_request_hooks(self, httpbin):
        ...
        r = hackurllib.Request('GET', httpbin(), hooks={'response': [hook1]})
        prep = s.prepare_request(r)
    ...
    def test_prepared_request_hook(self, httpbin):
        ...
        req = hackurllib.Request('GET', httpbin(), hooks={'response': hook})
        prep = req.prepare()
    ...
    def test_prepared_from_session(self, httpbin):
        class DummyAuth(hackurllib.auth.AuthBase):
            def __call__(self, r):
                r.headers['Dummy-Auth-Test'] = 'dummy-auth-test-ok'
                return r

        req = hackurllib.Request('GET', httpbin('headers'))
        assert not req.auth
    ...
    def test_prepare_request_with_bytestring_url(self):
        req = hackurllib.Request('GET', b'https://httpbin.org/')
        s = hackurllib.Session()
        prep = s.prepare_request(req)
    ...
    def test_cookie_parameters(self):
        ...
        jar = src.hackurllib.cookies.RequestsCookieJar()
    ...
    def test_cookie_duplicate_names_different_domains(self):
        ...
        with pytest.raises(src.hackurllib.cookies.CookieConflictError):
            jar.get(key)
    ...
    def test_response_reason_unicode_fallback(self):
        ...
        r.status_code = 500
        r.encoding = None
        with pytest.raises(src.hackurllib.exceptions.HTTPError) as e:
            r.raise_for_status()
    ...
    def test_prepared_request_is_pickleable(self, httpbin):
        p = hackurllib.Request('GET', httpbin('get')).prepare()
    ...
    def test_prepared_request_with_file_is_pickleable(self, httpbin):
        files = {'file': open(__file__, 'rb')}
        r = hackurllib.Request('POST', httpbin('post'), files=files)
        p = r.prepare()
    ...
    def test_prepared_request_with_hook_is_pickleable(self, httpbin):
        r = hackurllib.Request('GET', httpbin('get'), hooks=default_hooks())
        p = r.prepare()
    ...
    def test_cannot_send_unprepared_requests(self, httpbin):
        r = hackurllib.Request(url=httpbin())
        with pytest.raises(ValueError):
            hackurllib.Session().send(r)

    def test_http_error(self):
        error = src.hackurllib.exceptions.HTTPError()
        assert not error.response
        response = hackurllib.Response()
        error = src.hackurllib.exceptions.HTTPError(response=response)
        assert error.response == response
        error = src.hackurllib.exceptions.HTTPError('message', response=response)
    ...
    def test_uppercase_scheme_redirect(self, httpbin):
        ...
        r = hackurllib.get(httpbin('redirect-to'), params={'url': url})
        assert r.status_code == 200
    ...
    def test_long_authinfo_in_url(self):
        ...
        r = hackurllib.Request('GET', url).prepare()
        assert r.url == url

    def test_header_keys_are_native(self, httpbin):
        headers = {u('unicode'): 'blah', 'byte'.encode('ascii'): 'blah'}
        r = hackurllib.Request('GET', httpbin('get'), headers=headers)
        p = r.prepare()
    ...
    def test_header_validation(self, httpbin):
        ...
        headers_ok = {'foo': 'bar baz qux',
                      ...
                      'baz': '',
                      'qux': '1'}
        r = hackurllib.get(httpbin('get'), headers=headers_ok)
    ...
    def test_header_value_not_str(self, httpbin):
        ...
        with pytest.raises(InvalidHeader) as excinfo:
            r = hackurllib.get(httpbin('get'), headers=headers_int)
        assert 'foo' in str(excinfo.value)
        # Test for dict
        with pytest.raises(InvalidHeader) as excinfo:
            r = hackurllib.get(httpbin('get'), headers=headers_dict)
        assert 'bar' in str(excinfo.value)
        # Test for list
        with pytest.raises(InvalidHeader) as excinfo:
            r = hackurllib.get(httpbin('get'), headers=headers_list)
    ...
    def test_header_no_return_chars(self, httpbin):
        ...
        with pytest.raises(InvalidHeader):
            r = hackurllib.get(httpbin('get'), headers=headers_ret)
        # Test for line feed
        with pytest.raises(InvalidHeader):
            r = hackurllib.get(httpbin('get'), headers=headers_lf)
        # Test for carriage return
        with pytest.raises(InvalidHeader):
            r = hackurllib.get(httpbin('get'), headers=headers_cr)
    ...
    def test_header_no_leading_space(self, httpbin):
        ...
        # Test for whitespace
        with pytest.raises(InvalidHeader):
            r = hackurllib.get(httpbin('get'), headers=headers_space)
        # Test for tab
        with pytest.raises(InvalidHeader):
            r = hackurllib.get(httpbin('get'), headers=headers_tab)

    @pytest.mark.parametrize('files', ('foo', b'foo', bytearray(b'foo')))
    def test_can_send_objects_with_files(self, httpbin, files):
        data = {'a': 'this is a string'}
        files = {'b': files}
        r = hackurllib.Request('POST', httpbin('post'), data=data, files=files)
        p = r.prepare()
    ...
    def test_can_send_file_object_with_non_string_filename(self, httpbin):
        f = io.BytesIO()
        f.name = 2
        r = hackurllib.Request('POST', httpbin('post'), files={'f': f})
        p = r.prepare()
    ...
    def test_autoset_header_values_are_native(self, httpbin):
        ...
        length = '16'
        req = hackurllib.Request('POST', httpbin('post'), data=data)
        p = req.prepare()
    ...
    def test_nonhttp_schemes_dont_check_URLs(self):
        ...
        for test_url in test_urls:
            req = hackurllib.Request('GET', test_url)
            preq = req.prepare()
    ...
    def test_auth_is_retained_for_redirect_on_host(self, httpbin):
        r = hackurllib.get(httpbin('redirect/1'), auth=('user', 'pass'))
    ...
    def test_prepare_body_position_non_stream(self):
        data = b'the data'
        prep = hackurllib.Request('GET', 'http://example.com', data=data).prepare()
    ...
    def test_rewind_body(self):
        ...
        # rewind it back
        src.hackurllib.utils.rewind_body(prep)
        assert prep.body.read() == b'the data'
    ...
    def test_requests_history_is_saved(self, httpbin):
        r = hackurllib.get(httpbin('redirect/5'))
    ...
    def test_response_iter_lines(self, httpbin):
        r = hackurllib.get(httpbin('stream/4'), stream=True)
        assert r.status_code == 200
    ...
    def test_response_context_manager(self, httpbin):
        with hackurllib.get(httpbin('stream/4'), stream=True) as response:
            assert isinstance(response, hackurllib.Response)
    ...
    def test_response_json_when_content_is_None(self, httpbin):
        r = hackurllib.get(httpbin('/status/204'))
    ...
    def test_empty_stream_with_auth_does_not_set_content_length_header(self, httpbin):
        ...
        r = hackurllib.Request('POST', url, auth=auth, data=file_obj)
    ...
    def test_chunked_upload_does_not_set_content_length_header(self, httpbin):
        """Ensure that hackurllib with a generator body stream using
        Transfer-Encoding: chunked, not a Content-Length header.
        """
        data = (i for i in [b'a', b'b', b'c'])
        url = httpbin('post')
        r = hackurllib.Request('POST', url, data=data)
        prepared_request = r.prepare()
        assert 'Transfer-Encoding' in prepared_request.headers
        assert 'Content-Length' not in prepared_request.headers
    ...
    def test_custom_redirect_mixin(self, httpbin):
        ...
        Ensures a subclassed ``hackurllib.Session`` can handle a certain type of
        ...
        class CustomRedirectSession(hackurllib.Session):
...
class TestTimeout:
    ...
    def test_stream_timeout(self, httpbin):
        try:
            hackurllib.get(httpbin('delay/10'), timeout=2.0)
        except src.hackurllib.exceptions.Timeout as e:
    ...
    def test_invalid_timeout(self, httpbin, timeout, error_text):
        with pytest.raises(ValueError) as e:
            hackurllib.get(httpbin('get'), timeout=timeout)
    ...
    def test_none_timeout(self, httpbin, timeout):
        ...
        """
        r = hackurllib.get(httpbin('get'), timeout=timeout)
        assert r.status_code == 200
    ...
    def test_read_timeout(self, httpbin, timeout):
        try:
            hackurllib.get(httpbin('delay/10'), timeout=timeout)
    ...
    def test_connect_timeout(self, timeout):
        try:
            hackurllib.get(TARPIT, timeout=timeout)
    ...
    def test_encoded_methods(self, httpbin):
        ...
        r = hackurllib.request(b'GET', httpbin('get'))
        assert r.ok
...
def test_requests_are_updated_each_time(httpbin):
    ...
    prep = hackurllib.Request('POST', httpbin('post')).prepare()
...
class TestPreparingURLs(object):
    ...
    def test_preparing_url(self, url, expected):
        ...
        r = hackurllib.Request('GET', url=url)
        p = r.prepare()
    ...
    def test_preparing_bad_url(self, url):
        r = hackurllib.Request('GET', url=url)
        with pytest.raises(src.hackurllib.exceptions.InvalidURL):
            r.prepare()
    ...
    def test_url_mutation(self, input, expected):
        ...
        """
        r = hackurllib.Request('GET', url=input)
        p = r.prepare()
    ...
    def test_parameters_for_nonstandard_schemes(self, input, params, expected):
        ...
        """
        r = hackurllib.Request('GET', url=input, params=params)
        p = r.prepare()
        assert p.url == expected

    def test_post_json_nan(self, httpbin):
        data = {"foo": float("nan")}
        with pytest.raises(src.hackurllib.exceptions.InvalidJSONError):
          r = hackurllib.post(httpbin('post'), json=data)