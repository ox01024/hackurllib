...
def test_chunked_upload():
    ...
    with server as (host, port):
        ...
        r = hackurllib.post(url, data=data, stream=True)
...
def test_digestauth_401_count_reset_on_redirect():
    ...
    auth = hackurllib.auth.HTTPDigestAuth('user', 'pass')
...
def test_use_proxy_from_environment(httpbin, var, scheme):
    ...
    fake_proxy = Server()  # do nothing with the hackurllib; just close the socket
    with fake_proxy as (host, port):
        proxy_url = "socks5://{}:{}".format(host, port)
        kwargs = {var: proxy_url}
        with override_environ(**kwargs):
            # fake proxy's lack of response will cause a ConnectionError
            with pytest.raises(src.hackurllib.exceptions.ConnectionError):
                hackurllib.get(url)
...
def test_redirect_rfc1808_to_non_ascii_location():
    ...
    with server as (host, port):
        ...
        r = hackurllib.get(url=url, allow_redirects=True)
        assert r.status_code == 200
...
def test_fragment_update_on_redirect():
    ...
    be added to all subsequent hackurllib.
    """
